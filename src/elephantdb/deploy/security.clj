...
(ns ^{:author "Juegen Hoetzel, juergen@archlinux.org"
  ...
  elephantdb.deploy.security
  (:require [org.jclouds.compute2 :as compute]
            [org.jclouds.ec2.ebs :as ebs])
  (:import [org.jclouds.ec2.domain IpProtocol SecurityGroup UserIdGroupPair]
           org.jclouds.ec2.services.SecurityGroupClient
           java.io.DataInputStream
           java.net.URL))

(defn ^SecurityGroupClient sg-service
...
(defn create-group
  ...
  (.createSecurityGroupInRegion (sg-service compute)
...
(defn delete-group
  ...
  (.deleteSecurityGroupInRegion (sg-service compute)
                                (ebs/get-region region)
                                name))
  ...
  (let [group ((groups compute :region region) group-name)
    ...
    (if group
      (.authorizeSecurityGroupIngressInRegion
       ...
       from-port
       to-port
       (or ip-range "0.0.0.0/0"))
      (throw (IllegalArgumentException.
              (str "Can't find security group for name " group-name))))))

(def my-ip
  (memoize
   #(with-open [is (DataInputStream.
                    (.openStream
                     (URL. "http://demontunes.com/api/?g=ip")))]
      (.readLine is))))

(defn authorizeme [compute group-name port]
  (try (authorize compute group-name port :ip-range (str (my-ip) "/32"))
...
(defn authorize-group
  ...
  ([compute region to-group from-group user-id]
     (try (.authorizeSecurityGroupIngressInRegion
           (sg-service compute)
           region
           to-group
